...
Three of the four have moved, and the parameter half of the fourth; the inventory stands at 5.
`dsdl-mark-infallible-bodies` states
...
generated byte changed. What remains of the fourth is the size a body is handed by pointer. In Rust,
Go, TypeScript and Python a body takes a buffer that carries its length and answers the size it
used, as `dsdl.call_serdes_sized` already states from the caller's side; Rust alone still analyses
the pointer.
