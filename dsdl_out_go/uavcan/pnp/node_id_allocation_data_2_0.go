...
const NODE_ID_ALLOCATION_DATA_FULL_NAME = "uavcan.pnp.NodeIDAllocationData"
const NODE_ID_ALLOCATION_DATA_IS_DEPRECATED = false
const NODE_ID_ALLOCATION_DATA_FULL_NAME_AND_VERSION = "uavcan.pnp.NodeIDAllocationData.2.0"
const NODE_ID_ALLOCATION_DATA_EXTENT_BYTES = 48
const NODE_ID_ALLOCATION_DATA_SERIALIZATION_BUFFER_SIZE_BYTES = 18
const NODE_ID_ALLOCATION_DATA_ZOH_ALIAS_ELIGIBLE = false
const NODE_ID_ALLOCATION_DATA_ZOH_ALIAS_REASON = "invalid-bit-length"
...
func (obj *NodeIDAllocationData) Serialize(buffer []byte) (int8, int) {
	if obj == nil {
		return -dsdlruntime.DSDL_RUNTIME_ERROR_INVALID_ARGUMENT, 0
	}
	offsetBits := 0
	mlir_llvmdsdl_plan_capacity_check__uavcan_pnp_NodeIDAllocationData_2_0 := func(capacityBits int64) int8 {
		if 144 > capacityBits {
			return -dsdlruntime.DSDL_RUNTIME_ERROR_SERIALIZATION_BUFFER_TOO_SMALL
		}
		return dsdlruntime.DSDL_RUNTIME_SUCCESS
	}
	mlir_llvmdsdl_plan_scalar_unsigned__uavcan_pnp_NodeIDAllocationData_2_0__2__ser := func(value uint64) uint64 {
		if value > uint64(255) {
			return uint64(255)
		}
		return value
	}
	if rc := mlir_llvmdsdl_plan_capacity_check__uavcan_pnp_NodeIDAllocationData_2_0(int64(len(buffer) * 8)); rc != dsdlruntime.DSDL_RUNTIME_SUCCESS {
		return rc, 0
	}
	for (offsetBits % 8) != 0 {
	...
	_rc4_, _consumed5_ := obj.NodeId.Serialize(buffer[_start2_:_end3_])
	if _rc4_ < 0 {
		return _rc4_, 0
	}
	_sizeBytes1_ = _consumed5_
	offsetBits += _sizeBytes1_ * 8
	for _index6_ := 0; _index6_ < 16; _index6_++ {
		_err7_ := dsdlruntime.SetUxx(buffer, offsetBits, mlir_llvmdsdl_plan_scalar_unsigned__uavcan_pnp_NodeIDAllocationData_2_0__2__ser(uint64(obj.UniqueId[_index6_])), 8)
		if _err7_ < 0 {
			return _err7_, 0
		}
		offsetBits += 8
...
func (obj *NodeIDAllocationData) Deserialize(buffer []byte) (int8, int) {
	if obj == nil {
		return -dsdlruntime.DSDL_RUNTIME_ERROR_INVALID_ARGUMENT, 0
	}
	...
	offsetBits := 0
	mlir_llvmdsdl_plan_scalar_unsigned__uavcan_pnp_NodeIDAllocationData_2_0__2__deser := func(value uint64) uint64 { return value & uint64(255) }
	offsetBits = (offsetBits + 7) & ^7
	_start9_ := dsdlruntime.ChooseMin(offsetBits/8, len(buffer))
	_rc10_, _consumed11_ := obj.NodeId.Deserialize(buffer[_start9_:len(buffer)])
	if _rc10_ < 0 {
		return _rc10_, 0
	}
	...
	for _index13_ := 0; _index13_ < _count12_; _index13_++ {
		_raw14_ := uint64(dsdlruntime.GetU64(buffer, offsetBits, 8))
		obj.UniqueId[_index13_] = uint8(mlir_llvmdsdl_plan_scalar_unsigned__uavcan_pnp_NodeIDAllocationData_2_0__2__deser(_raw14_))
		offsetBits += 8
	}
	offsetBits = (offsetBits + 7) & ^7
