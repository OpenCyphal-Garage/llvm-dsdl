...
`readable_bytes`, which stands a byte in for a span that holds no pointer. The `std`, `pmr` and
`autosar` profiles differ in how those containers and that span are spelled — `std::span`, or
CETL's `cetl::pf20::span` under `autosar` — and, under `pmr`, in the memory resource a nested call
is handed. The C↔C++ parity lanes and the generation lane accept it.
