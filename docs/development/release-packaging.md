...
Built in [packaging/docker/Dockerfile.ubuntu-release](https://github.com/OpenCyphal-Garage/llvm-dsdl/blob/main/packaging/docker/Dockerfile.ubuntu-release).
...
(Ubuntu 26.04), so [include/llvmdsdl/Support/FlatSet.h](https://github.com/OpenCyphal-Garage/llvm-dsdl/blob/main/include/llvmdsdl/Support/FlatSet.h)
...
- **macOS** — [cmake/BundleSelfContainedTools.cmake](https://github.com/OpenCyphal-Garage/llvm-dsdl/blob/main/cmake/BundleSelfContainedTools.cmake)
...
[packaging/deb/copyright](https://github.com/OpenCyphal-Garage/llvm-dsdl/blob/main/packaging/deb/copyright) carries the stanza covering the shipped
library.
...
[packaging/deb/derive_depends.py](https://github.com/OpenCyphal-Garage/llvm-dsdl/blob/main/packaging/deb/derive_depends.py): walk what the tools
...
|---|---|---|
| [smoke.py](https://github.com/OpenCyphal-Garage/llvm-dsdl/blob/main/packaging/verify/smoke.py) | The `.deb` installs on **pristine** Ubuntu with no apt.llvm.org, all three tools run, `libLLVM` resolves through RPATH, generated C compiles with stock `cc`, a non-C backend emits | release workflow |
| [smoke_macos.py](https://github.com/OpenCyphal-Garage/llvm-dsdl/blob/main/packaging/verify/smoke_macos.py) | Every non-system reference in every binary and dylib resolves **inside the bundle**, tools run, generated C compiles | release workflow |
...
3. **TableGen must run on the build host.** [include/llvmdsdl/IR/CMakeLists.txt](https://github.com/OpenCyphal-Garage/llvm-dsdl/blob/main/include/llvmdsdl/IR/CMakeLists.txt)
