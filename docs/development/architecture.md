...
5. translation of those functions, one per backend: `convert-dsdl-to-llvm` and `emit-dsdl-runtime`
   for objects, a translator and a spelling per language for the source backends. A backend is this translation and nothing more; `ctest -L backend-contract` reports which
