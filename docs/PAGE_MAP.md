# Page Map

This manual is organized for three reader paths: users, backend integrators, and contributors.

## Information architecture

1. Home
2. Getting Started
3. CLI
4. Backends
5. Design
6. Validation
7. LSP Guides
8. Development
9. Project map

## Navigation map

- Home
  - `index.md`
- Getting Started
  - `getting-started/install.md`
  - `getting-started/quickstart.md`
- CLI
  - `cli/dsdlc.md`
  - `cli/dsdl-opt.md`
  - `cli/dsdld.md`
- Backends
  - `backends/overview.md`
  - `backends/object.md`
- Showroom (generated by the `showroom-docs` target, gitignored, produced by the Docs workflow)
  - `showroom/index.md` (rendered from `examples/showroom/README.md`)
  - `showroom/types/*.md` (reachable from the overview, not listed in `nav`)
- Design
  - `design/architecture.md`
  - `design/dialect-contracts.md`
- Validation
  - `PARITY_MATRIX.md`
  - `MALFORMED_INPUT_CONTRACT_MATRIX.md`
  - `CONVERGENCE_SCORECARD.md`
- LSP Guides
  - `LSP_AI_OPERATOR_GUIDE.md`
  - `LSP_INDEX_SCHEMA.md`
  - `LSP_LINT_RULES.md`
  - `LSP_LINT_RULE_AUTHORING.md`
  - `LSP_RANKING_MODEL.md`
- Development
  - `development/contributing.md`
  - `development/testing.md`
- Project
  - `PAGE_MAP.md`

## Planned growth

- Auto-generated CLI reference snapshot pages from `dsdlc --help`
- Auto-generated MLIR dialect op reference pages
- Versioned documentation when release cadence requires it
