...
file(WRITE
  "${fixture_root}/demo/OuterArray.1.0.dsdl"
...
execute_process(
  COMMAND
    "${DSDLC}" --target-language ts --versioned-type-names
      "${fixture_root}"
      --outdir "${ts_out}"
      --ts-module "ts_runtime_composite_fixed_array_smoke"
...
if(NOT ts_gen_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript generation failed for composite fixed-array smoke")
endif()
...
file(WRITE
  "${ts_out}/runtime_composite_fixed_array_smoke.ts"
...
file(WRITE
  "${ts_out}/tsconfig-runtime-composite-fixed-array-smoke.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-composite-fixed-array-smoke.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile composite fixed-array TypeScript runtime smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_composite_fixed_array_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript composite fixed-array runtime smoke execution failed")
endif()

string(STRIP "${node_stdout}" actual_output)
set(expected_output "17 34 42\n17 34 42 3")
if(NOT actual_output STREQUAL expected_output)
  file(WRITE "${OUT_DIR}/expected-output.txt" "${expected_output}\n")
  file(WRITE "${OUT_DIR}/actual-output.txt" "${actual_output}\n")
  message(FATAL_ERROR
    "unexpected composite fixed-array runtime smoke output. See ${OUT_DIR}/expected-output.txt and ${OUT_DIR}/actual-output.txt.")
endif()

message(STATUS "TypeScript composite fixed-array runtime smoke passed")
