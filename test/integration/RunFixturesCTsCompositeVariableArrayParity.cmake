...
file(WRITE
  "${fixture_root}/OuterVarArray.1.0.dsdl"
...
set(c_harness_src "${work_dir}/c_composite_variable_array_parity_harness.c")
file(WRITE
  "${c_harness_src}"
  "#include <stdint.h>\n"
  "#include <stdio.h>\n"
  "#include \"demo/Inner_1_0.h\"\n"
  "#include \"demo/OuterVarArray_1_0.h\"\n"
  "\n"
  "int main(void) {\n"
  "  demo__OuterVarArray in_obj;\n"
  "  in_obj.items.count = 2U;\n"
  "  in_obj.items.elements[0].x = 17U;\n"
  "  in_obj.items.elements[1].x = 34U;\n"
  "  in_obj.tail = 42U;\n"
  "\n"
  "  uint8_t out_bytes[8] = {0};\n"
  "  size_t out_size = sizeof(out_bytes);\n"
  "  const int8_t ser_rc = demo__OuterVarArray__serialize_(&in_obj, out_bytes, &out_size);\n"
  "  if ((ser_rc != 0) || (out_size != 4U)) {\n"
  "    return 2;\n"
  "  }\n"
  "  printf(\"%u %u %u %u\\n\", (unsigned) out_bytes[0], (unsigned) out_bytes[1], (unsigned) out_bytes[2], (unsigned) out_bytes[3]);\n"
  "\n"
  "  demo__OuterVarArray out_obj;\n"
  "  out_obj.items.count = 0U;\n"
  "  out_obj.tail = 0U;\n"
  "  size_t consumed = out_size;\n"
  "  const int8_t de_rc = demo__OuterVarArray__deserialize_(&out_obj, out_bytes, &consumed);\n"
  "  if ((de_rc != 0) || (consumed != 4U) || (out_obj.items.count != 2U)) {\n"
  "    return 3;\n"
  "  }\n"
  "  printf(\"%u %u %u %u %u\\n\", (unsigned) out_obj.items.count, (unsigned) out_obj.items.elements[0].x, (unsigned) out_obj.items.elements[1].x, (unsigned) out_obj.tail, (unsigned) consumed);\n"
  "  return 0;\n"
  "}\n"
)

set(c_harness_bin "${work_dir}/c_composite_variable_array_parity_harness")
execute_process(
  COMMAND
    "${C_COMPILER}"
      -std=c11
      ...
      "${c_out}/demo/OuterVarArray_1_0.c"
      -o "${c_harness_bin}"
...
if(NOT c_cc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture C composite variable-array parity harness")
endif()
...
if(NOT c_run_result EQUAL 0)
  ...
  message(FATAL_ERROR "fixture C composite variable-array parity harness failed")
endif()

file(WRITE
  "${ts_out}/runtime_composite_variable_array_parity_smoke.ts"
  ...
  "if (decoded.value.items.length !== 2) {\n"
  "  throw new Error(\"unexpected decoded array length \" + decoded.value.items.length);\n"
  "}\n"
...
file(WRITE
  "${ts_out}/tsconfig-runtime-composite-variable-array-parity.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-composite-variable-array-parity.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture TypeScript composite variable-array parity smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_composite_variable_array_parity_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript composite variable-array parity smoke execution failed")
endif()
...
if(NOT c_output STREQUAL ts_output)
  ...
  message(FATAL_ERROR
    "C vs TypeScript composite variable-array parity mismatch. See ${OUT_DIR}/c-output.txt and ${OUT_DIR}/ts-output.txt.")
endif()

message(STATUS "fixture C<->TypeScript composite variable-array parity smoke passed")
