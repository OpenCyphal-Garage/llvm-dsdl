...
if(NOT DEFINED FIXTURE_ROOT OR "${FIXTURE_ROOT}" STREQUAL "")
  set(FIXTURE_ROOT "${SOURCE_ROOT}/test/lit/fixtures/vendor")
endif()
if(NOT EXISTS "${FIXTURE_ROOT}")
  message(FATAL_ERROR "fixture root not found: ${FIXTURE_ROOT}")
endif()
...
execute_process(
  COMMAND
    "${DSDLC}" --target-language ts
      "${FIXTURE_ROOT}"
      --outdir "${ts_out}"
      --ts-module "fixture_ts_runtime_parity"
...
set(c_harness_src "${work_dir}/c_parity_harness.c")
file(WRITE
  "${c_harness_src}"
  "#include <stdint.h>\n"
  "#include <stdio.h>\n"
...
set(c_harness_bin "${work_dir}/c_parity_harness")
execute_process(
  COMMAND
    "${C_COMPILER}"
      -std=c11
...
if(NOT c_cc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture C parity harness")
endif()
...
if(NOT c_run_result EQUAL 0)
  ...
  message(FATAL_ERROR "fixture C parity harness failed")
endif()
...
file(WRITE
  "${ts_out}/tsconfig-runtime-parity.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-parity.json" --pretty false
...
execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_parity_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript runtime parity smoke execution failed")
endif()
...
if(NOT c_output STREQUAL ts_output)
  ...
  message(FATAL_ERROR
    "C vs TypeScript runtime parity mismatch. See ${OUT_DIR}/c-output.txt and ${OUT_DIR}/ts-output.txt.")
endif()

message(STATUS "fixture C<->TypeScript runtime parity smoke passed")
