...
string(REGEX MATCH "export \\* as ([A-Za-z0-9_]+) from \"\\./[^\"]*var_array_1_0\";" ts_type_export "${ts_index_content}")
if("${CMAKE_MATCH_1}" STREQUAL "")
  message(FATAL_ERROR "failed to locate var_array_1_0 export alias in ${ts_index}")
endif()
set(ts_type_module "${CMAKE_MATCH_1}")

set(c_harness_src "${work_dir}/c_var_array_parity_harness.c")
file(WRITE
  "${c_harness_src}"
  "#include <stdint.h>\n"
  "#include <stdio.h>\n"
  ...
  "  const int8_t ser_rc = demo__VarArray__serialize_(&in_obj, out_bytes, &out_size);\n"
  "  if ((ser_rc != 0) || (out_size != 3U)) {\n"
  "    return 2;\n"
  "  }\n"
  "  printf(\"%u %u %u\\n\", (unsigned) out_bytes[0], (unsigned) out_bytes[1], (unsigned) out_bytes[2]);\n"
  "\n"
  "  demo__VarArray out_obj;\n"
  "  out_obj.values.count = 0U;\n"
  "  out_obj.tail = 0U;\n"
  "  size_t consumed = out_size;\n"
  "  const int8_t de_rc = demo__VarArray__deserialize_(&out_obj, out_bytes, &consumed);\n"
  "  if ((de_rc != 0) || (consumed != 3U) || (out_obj.values.count != 3U)) {\n"
  "    return 3;\n"
  "  }\n"
  "  printf(\"%u %u %u %u %u %u\\n\", (unsigned) out_obj.values.count, (unsigned) out_obj.values.elements[0], (unsigned) out_obj.values.elements[1], (unsigned) out_obj.values.elements[2], (unsigned) out_obj.tail, (unsigned) consumed);\n"
  "  return 0;\n"
  "}\n"
)

set(c_harness_bin "${work_dir}/c_var_array_parity_harness")
execute_process(
  COMMAND
    "${C_COMPILER}"
      -std=c11
...
if(NOT c_cc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture C variable-array parity harness")
endif()
...
if(NOT c_run_result EQUAL 0)
  ...
  message(FATAL_ERROR "fixture C variable-array parity harness failed")
endif()

file(WRITE
  "${ts_out}/runtime_variable_array_parity_smoke.ts"
  "import { ${ts_type_module} } from \"./index\";\n"
  "\n"
  "const inObj: ${ts_type_module}.VarArray_1_0 = { values: [1, 6, 8], tail: 63 };\n"
  ...
  "if (decoded.value.values.length !== 3) {\n"
  "  throw new Error(\"unexpected decoded array length \" + decoded.value.values.length);\n"
  "}\n"
  "console.log(decoded.value.values.length + \" \" + decoded.value.values[0] + \" \" + decoded.value.values[1] + \" \" + decoded.value.values[2] + \" \" + decoded.value.tail + \" \" + decoded.consumed);\n"
)

file(WRITE
  "${ts_out}/tsconfig-runtime-variable-array-parity.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-variable-array-parity.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture TypeScript variable-array parity smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_variable_array_parity_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript variable-array parity smoke execution failed")
endif()
...
if(NOT c_output STREQUAL ts_output)
  ...
  message(FATAL_ERROR
    "C vs TypeScript variable-array parity mismatch. See ${OUT_DIR}/c-output.txt and ${OUT_DIR}/ts-output.txt.")
endif()

message(STATUS "fixture C<->TypeScript variable-array parity smoke passed")
