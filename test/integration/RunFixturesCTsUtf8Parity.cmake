...
file(WRITE
  "${fixture_root}/demo/Utf8Type.1.0.dsdl"
  "utf8 ch\n"
  "utf8[2] text\n"
  "@sealed\n"
)
...
execute_process(
  COMMAND
    "${DSDLC}" --target-language ts --versioned-type-names
      "${fixture_root}"
      --outdir "${ts_out}"
      --ts-module "fixture_ts_utf8_parity"
...
string(REGEX MATCH "export \\* as ([A-Za-z0-9_]+) from \"\\./[^\"]+\";" ts_type_export "${ts_index_content}")
if("${CMAKE_MATCH_1}" STREQUAL "")
  message(FATAL_ERROR "failed to locate exported TypeScript alias in ${ts_index}")
endif()
set(ts_type_module "${CMAKE_MATCH_1}")

set(c_harness_src "${work_dir}/c_utf8_parity_harness.c")
file(WRITE
  "${c_harness_src}"
  "#include <stdint.h>\n"
  "#include <stdio.h>\n"
  "#include \"fixture_root/demo/Utf8Type_1_0.h\"\n"
  "\n"
  "int main(void) {\n"
  "  fixture_root__demo__Utf8Type in_obj;\n"
  "  in_obj.ch = 65U;\n"
  "  in_obj.text[0] = 66U;\n"
  "  in_obj.text[1] = 67U;\n"
  "\n"
  ...
  "  const int8_t ser_rc = fixture_root__demo__Utf8Type__serialize_(&in_obj, out_bytes, &out_size);\n"
  "  if ((ser_rc != 0) || (out_size != 3U)) {\n"
  "    return 2;\n"
  "  }\n"
  "  printf(\"%u %u %u\\n\", (unsigned) out_bytes[0], (unsigned) out_bytes[1], (unsigned) out_bytes[2]);\n"
  "\n"
  "  fixture_root__demo__Utf8Type out_obj;\n"
  "  out_obj.ch = 0U;\n"
  "  out_obj.text[0] = 0U;\n"
  "  out_obj.text[1] = 0U;\n"
  "  size_t consumed = out_size;\n"
  "  const int8_t de_rc = fixture_root__demo__Utf8Type__deserialize_(&out_obj, out_bytes, &consumed);\n"
  "  if ((de_rc != 0) || (consumed != 3U)) {\n"
  "    return 3;\n"
  "  }\n"
  "  printf(\"%u %u %u %u\\n\", (unsigned) out_obj.ch, (unsigned) out_obj.text[0], (unsigned) out_obj.text[1], (unsigned) consumed);\n"
  "  return 0;\n"
  "}\n"
)

set(c_harness_bin "${work_dir}/c_utf8_parity_harness")
execute_process(
  COMMAND
    "${C_COMPILER}"
      -std=c11
      ...
      -I "${c_out}"
      "${c_harness_src}"
      "${c_out}/fixture_root/demo/Utf8Type_1_0.c"
      -o "${c_harness_bin}"
...
if(NOT c_cc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture C utf8 parity harness")
endif()
...
if(NOT c_run_result EQUAL 0)
  ...
  message(FATAL_ERROR "fixture C utf8 parity harness failed")
endif()

file(WRITE
  "${ts_out}/runtime_utf8_parity_smoke.ts"
...
file(WRITE
  "${ts_out}/tsconfig-runtime-utf8-parity.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-utf8-parity.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture TypeScript utf8 parity smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_utf8_parity_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript utf8 parity smoke execution failed")
endif()
...
if(NOT c_output STREQUAL ts_output)
  ...
  message(FATAL_ERROR
    "C vs TypeScript utf8 parity mismatch. See ${OUT_DIR}/c-output.txt and ${OUT_DIR}/ts-output.txt.")
endif()

message(STATUS "fixture C<->TypeScript utf8 parity smoke passed")
