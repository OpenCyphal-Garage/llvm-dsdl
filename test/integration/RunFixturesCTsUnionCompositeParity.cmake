...
file(WRITE
  "${fixture_root}/UnionComposite.1.0.dsdl"
  "@union\n"
...
execute_process(
  COMMAND
    "${DSDLC}" --target-language ts --versioned-type-names
      "${fixture_root}"
      --outdir "${ts_out}"
      --ts-module "fixture_ts_union_composite_parity"
...
set(c_harness_src "${work_dir}/c_union_composite_parity_harness.c")
file(WRITE
  "${c_harness_src}"
  "#include <stdint.h>\n"
  "#include <stdio.h>\n"
  "#include \"demo/Inner_1_0.h\"\n"
  "#include \"demo/UnionComposite_1_0.h\"\n"
  "\n"
  "int main(void) {\n"
  "  demo__UnionComposite in_obj;\n"
  "  in_obj._tag_ = 0U;\n"
  "  in_obj.inner.a = 18U;\n"
  "  in_obj.inner.b = 6U;\n"
  "\n"
  ...
  "  const int8_t ser_rc = demo__UnionComposite__serialize_(&in_obj, out_bytes, &out_size);\n"
  "  if ((ser_rc != 0) || (out_size != 3U)) {\n"
  "    return 2;\n"
  "  }\n"
  "  printf(\"%u %u %u\\n\", (unsigned) out_bytes[0], (unsigned) out_bytes[1], (unsigned) out_bytes[2]);\n"
  "\n"
  "  demo__UnionComposite out_obj;\n"
  "  out_obj._tag_ = 1U;\n"
  "  out_obj.flag = 0U;\n"
  "  size_t consumed = out_size;\n"
  "  const int8_t de_rc = demo__UnionComposite__deserialize_(&out_obj, out_bytes, &consumed);\n"
  "  if ((de_rc != 0) || (consumed != 3U) || (out_obj._tag_ != 0U)) {\n"
  "    return 3;\n"
  "  }\n"
  "  printf(\"%u %u %u %u\\n\", (unsigned) out_obj._tag_, (unsigned) out_obj.inner.a, (unsigned) out_obj.inner.b, (unsigned) consumed);\n"
  "  return 0;\n"
  "}\n"
)

set(c_harness_bin "${work_dir}/c_union_composite_parity_harness")
execute_process(
  COMMAND
    "${C_COMPILER}"
      -std=c11
      ...
      "${c_out}/demo/UnionComposite_1_0.c"
      -o "${c_harness_bin}"
...
if(NOT c_cc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture C union composite parity harness")
endif()
...
if(NOT c_run_result EQUAL 0)
  ...
  message(FATAL_ERROR "fixture C union composite parity harness failed")
endif()

file(WRITE
  "${ts_out}/runtime_union_composite_parity_smoke.ts"
...
file(WRITE
  "${ts_out}/tsconfig-runtime-union-composite-parity.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-union-composite-parity.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture TypeScript union composite parity smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_union_composite_parity_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript union composite parity smoke execution failed")
endif()
...
if(NOT c_output STREQUAL ts_output)
  ...
  message(FATAL_ERROR
    "C vs TypeScript union composite parity mismatch. See ${OUT_DIR}/c-output.txt and ${OUT_DIR}/ts-output.txt.")
endif()

message(STATUS "fixture C<->TypeScript union composite parity smoke passed")
