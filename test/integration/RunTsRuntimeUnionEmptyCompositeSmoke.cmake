...
execute_process(
  COMMAND
    "${DSDLC}" --target-language ts --versioned-type-names
      "${fixture_root}"
      --outdir "${ts_out}"
      --ts-module "ts_runtime_union_empty_composite_smoke"
...
if(NOT ts_gen_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript generation failed for union-empty-composite smoke")
endif()
...
file(WRITE
  "${ts_out}/runtime_union_empty_composite_smoke.ts"
...
file(WRITE
  "${ts_out}/tsconfig-runtime-union-empty-composite-smoke.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-union-empty-composite-smoke.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile union-empty-composite TypeScript runtime smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_union_empty_composite_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript union-empty-composite runtime smoke execution failed")
endif()
...
if(NOT actual_output STREQUAL expected_output)
  ...
  message(FATAL_ERROR
    "unexpected union-empty-composite runtime smoke output. See ${OUT_DIR}/expected-output.txt and ${OUT_DIR}/actual-output.txt.")
endif()

message(STATUS "TypeScript union-empty-composite runtime smoke passed")
