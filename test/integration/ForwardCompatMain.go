...
// Runtime forward/backward compatibility test for the generated Go serialisers. See
...
func main() {
	...
	buf := make([]byte, 64)
	n, err := wideIn.Serialize(buf)
	if err != nil {
		fail("go_serialize_wide_failed", 2)
	}
	var narrowOut nar.Holder@V1_0@
	if _, err := narrowOut.Deserialize(buf[:n]); err != nil {
	...
	buf2 := make([]byte, 64)
	n3, err := narrowIn.Serialize(buf2)
	if err != nil {
	...
	if _, err := wideOut.Deserialize(buf2[:n3]); err != nil {
