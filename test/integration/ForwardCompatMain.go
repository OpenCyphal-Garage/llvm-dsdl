...
func main() {
	...
	wideIn := wid.Holder_1_0{Inner: wid.Inner_1_0{X: 1, Y: 2}, Tail: 99}
	buf := make([]byte, 64)
	rc, n := wideIn.Serialize(buf)
	if rc != 0 {
		fail("go_serialize_wide_failed", 2)
	}
	var narrowOut nar.Holder_1_0
	...
	narrowIn := nar.Holder_1_0{Inner: nar.Inner_1_0{X: 5}, Tail: 77}
	buf2 := make([]byte, 64)
	...
	wideOut := wid.Holder_1_0{Inner: wid.Inner_1_0{X: 0xEE, Y: 0xEE}, Tail: 0xEE}
