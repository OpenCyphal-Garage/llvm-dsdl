...
#define FAIL(what, ...) do { if (++failures <= 3) printf("  " what "\n", __VA_ARGS__); } while (0)
...
int main(void)
{
    ...
    for (int t = 0; t < 40000; ++t) {
            ...
            if (rc_c != rc_i || memcmp(a,b,sizeof a)) {
                if (++failures <= 2) {
                    printf("set_uxx: size=%zu off=%zu len=%u val=%llx rc %d/%d\n",
                           size, off, len, (unsigned long long) val, rc_c, rc_i);
                    printf("   C : "); for(int k=0;k<(int)sizeof a;++k) printf("%02X",a[k]); printf("\n");
                    printf("   IR: "); for(int k=0;k<(int)sizeof b;++k) printf("%02X",b[k]); printf("\n");
                }
            }
        }
        memcpy(b,a,sizeof a);
            ...
            if (rc_c != rc_i || memcmp(a,b,sizeof a)) {
                if (++failures <= 2) {
                    printf("set_ixx: size=%zu off=%zu len=%u rc %d/%d  buffers %s\n",
                           size, off, len, rc_c, rc_i, memcmp(a,b,sizeof a) ? "DIFFER" : "equal");
                }
            }
        }
        memcpy(b,a,sizeof a);
        ...
        {
            uint8_t before[24]; memcpy(before,a,sizeof before);
            ...
            if (rc_c != rc_i || memcmp(a,b,sizeof a)) {
                if (++failures <= 2) {
                    ...
                    printf("   in: "); for(int k=0;k<8;++k) printf("%02X",before[k]); printf("\n");
                    printf("   C : "); for(int k=0;k<8;++k) printf("%02X",a[k]); printf("\n");
                    printf("   IR: "); for(int k=0;k<8;++k) printf("%02X",b[k]); printf("\n");
                }
            }
        }

        if (dsdl_runtime_get_u8 (buf,size,off,len) != ir_dsdl_runtime_get_u8 (buf,size,off,len)) FAIL("get_u8  differs: size=%zu off=%zu len=%u", size, off, len);
        if (dsdl_runtime_get_u16(buf,size,off,len) != ir_dsdl_runtime_get_u16(buf,size,off,len)) FAIL("get_u16 differs: size=%zu off=%zu len=%u", size, off, len);
        if (dsdl_runtime_get_u32(buf,size,off,len) != ir_dsdl_runtime_get_u32(buf,size,off,len)) FAIL("get_u32 differs: size=%zu off=%zu len=%u", size, off, len);
        if (dsdl_runtime_get_u64(buf,size,off,len) != ir_dsdl_runtime_get_u64(buf,size,off,len)) FAIL("get_u64 differs: size=%zu off=%zu len=%u", size, off, len);
        if (dsdl_runtime_get_i8 (buf,size,off,len) != ir_dsdl_runtime_get_i8 (buf,size,off,len)) FAIL("get_i8  differs: size=%zu off=%zu len=%u", size, off, len);
        if (dsdl_runtime_get_i16(buf,size,off,len) != ir_dsdl_runtime_get_i16(buf,size,off,len)) FAIL("get_i16 differs: size=%zu off=%zu len=%u", size, off, len);
        if (dsdl_runtime_get_i32(buf,size,off,len) != ir_dsdl_runtime_get_i32(buf,size,off,len)) FAIL("get_i32 differs: size=%zu off=%zu len=%u", size, off, len);
        if (dsdl_runtime_get_i64(buf,size,off,len) != ir_dsdl_runtime_get_i64(buf,size,off,len)) FAIL("get_i64 differs: size=%zu off=%zu len=%u", size, off, len);
        if (dsdl_runtime_get_bit(buf,size,off)     != ir_dsdl_runtime_get_bit(buf,size,off))     FAIL("get_bit differs: size=%zu off=%zu", size, off);
        ...
        if (memcmp(o1,o2,(glen+7)/8)) FAIL("get_bits differs: size=%zu off=%zu len=%zu", size, off, glen);
    }
