...
static double bitsd(uint64_t u)
{
    double d;
    memcpy(&d, &u, 8);
    return d;
...
int main(void)
{
    ...
    for (int t = 0; t < 60000; ++t)
    {
        ...
        float        f    = (t < (int) (sizeof specials / sizeof specials[0]))
                                ? specials[t]
                                : bitsf(((uint32_t) rnd(0xFFFFu) << 16) | rnd(0xFFFFu));
        double       d    = bitsd(((uint64_t) rnd(0xFFFFFFFFu) << 32) | rnd(0xFFFFFFFFu));

        int8_t rc, ri;
        rc = dsdl_runtime_set_f16(a, size, off, f);
        ri = ir_dsdl_runtime_set_f16(b, size, off, f);
        if (rc != ri || memcmp(a, b, sizeof a))
        {
            if (++failures <= 3)
                printf("set_f16 differs: size=%zu off=%zu f=%a\n", size, off, (double) f);
        }
        memcpy(b, a, sizeof a);
        rc = dsdl_runtime_set_f32(a, size, off, f);
        ri = ir_dsdl_runtime_set_f32(b, size, off, f);
        if (rc != ri || memcmp(a, b, sizeof a))
        {
            if (++failures <= 3)
                printf("set_f32 differs: size=%zu off=%zu f=%a\n", size, off, (double) f);
        }
        memcpy(b, a, sizeof a);
        rc = dsdl_runtime_set_f64(a, size, off, d);
        ri = ir_dsdl_runtime_set_f64(b, size, off, d);
        if (rc != ri || memcmp(a, b, sizeof a))
        {
            if (++failures <= 3)
                printf("set_f64 differs: size=%zu off=%zu d=%a\n", size, off, d);
        }
        ...
        if (memcmp(&c16, &i16v, 4))
        {
            if (++failures <= 3)
                printf("get_f16 differs: size=%zu off=%zu C=%a IR=%a\n", size, off, (double) c16, (double) i16v);
        }
        if (memcmp(&c32, &i32v, 4))
        {
            if (++failures <= 3)
                printf("get_f32 differs: size=%zu off=%zu C=%a IR=%a\n", size, off, (double) c32, (double) i32v);
        }
        if (memcmp(&c64, &i64v, 8))
        {
            if (++failures <= 3)
                printf("get_f64 differs: size=%zu off=%zu C=%a IR=%a\n", size, off, c64, i64v);
        }
    }
    if (failures)
    {
