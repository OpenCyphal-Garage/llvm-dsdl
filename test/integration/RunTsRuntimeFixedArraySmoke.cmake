...
file(WRITE
  "${fixture_root}/demo/FixedArray.1.0.dsdl"
  "uint8[3] values\n"
  "@sealed\n"
)
...
if(NOT ts_gen_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript generation failed for fixed-array smoke")
endif()
...
string(REGEX MATCH "export \\* as ([A-Za-z0-9_]+) from \"\\./[^\"]*fixed_array_1_0\";" ts_type_export "${ts_index_content}")
if("${CMAKE_MATCH_1}" STREQUAL "")
  message(FATAL_ERROR "failed to locate fixed_array_1_0 export alias in ${ts_index}")
endif()
set(ts_type_module "${CMAKE_MATCH_1}")

file(WRITE
  "${ts_out}/runtime_fixed_array_smoke.ts"
...
file(WRITE
  "${ts_out}/tsconfig-runtime-fixed-array-smoke.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-fixed-array-smoke.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixed-array TypeScript runtime smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_fixed_array_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript fixed-array runtime smoke execution failed")
endif()
...
if(NOT actual_output STREQUAL expected_output)
  ...
  message(FATAL_ERROR
    "unexpected fixed-array runtime smoke output. See ${OUT_DIR}/expected-output.txt and ${OUT_DIR}/actual-output.txt.")
endif()

message(STATUS "TypeScript fixed-array runtime smoke passed")
