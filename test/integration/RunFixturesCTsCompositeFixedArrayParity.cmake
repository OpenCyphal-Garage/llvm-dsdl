...
string(REGEX MATCH "export \\* as ([A-Za-z0-9_]+) from \"\\./[^\"]*outer_array_1_0\";" ts_type_export "${ts_index_content}")
if("${CMAKE_MATCH_1}" STREQUAL "")
  message(FATAL_ERROR "failed to locate outer_array_1_0 export alias in ${ts_index}")
endif()
set(ts_type_module "${CMAKE_MATCH_1}")

set(c_harness_src "${work_dir}/c_composite_fixed_array_parity_harness.c")
file(WRITE
  "${c_harness_src}"
  "#include <stdint.h>\n"
  "#include <stdio.h>\n"
  ...
  "  const int8_t ser_rc = demo__OuterArray__serialize_(&in_obj, out_bytes, &out_size);\n"
  "  if ((ser_rc != 0) || (out_size != 3U)) {\n"
  "    return 2;\n"
  "  }\n"
  "  printf(\"%u %u %u\\n\", (unsigned) out_bytes[0], (unsigned) out_bytes[1], (unsigned) out_bytes[2]);\n"
  "\n"
  "  demo__OuterArray out_obj;\n"
  "  out_obj.items[0].x = 0U;\n"
  "  out_obj.items[1].x = 0U;\n"
  "  out_obj.tail = 0U;\n"
  "  size_t consumed = out_size;\n"
  "  const int8_t de_rc = demo__OuterArray__deserialize_(&out_obj, out_bytes, &consumed);\n"
  "  if ((de_rc != 0) || (consumed != 3U)) {\n"
  "    return 3;\n"
  "  }\n"
  "  printf(\"%u %u %u %u\\n\", (unsigned) out_obj.items[0].x, (unsigned) out_obj.items[1].x, (unsigned) out_obj.tail, (unsigned) consumed);\n"
  "  return 0;\n"
  "}\n"
)

set(c_harness_bin "${work_dir}/c_composite_fixed_array_parity_harness")
execute_process(
  COMMAND
    "${C_COMPILER}"
      -std=c11
...
if(NOT c_cc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture C composite fixed-array parity harness")
endif()
...
if(NOT c_run_result EQUAL 0)
  ...
  message(FATAL_ERROR "fixture C composite fixed-array parity harness failed")
endif()

file(WRITE
  "${ts_out}/runtime_composite_fixed_array_parity_smoke.ts"
...
file(WRITE
  "${ts_out}/tsconfig-runtime-composite-fixed-array-parity.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-composite-fixed-array-parity.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture TypeScript composite fixed-array parity smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_composite_fixed_array_parity_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript composite fixed-array parity smoke execution failed")
endif()
...
if(NOT c_output STREQUAL ts_output)
  ...
  message(FATAL_ERROR
    "C vs TypeScript composite fixed-array parity mismatch. See ${OUT_DIR}/c-output.txt and ${OUT_DIR}/ts-output.txt.")
endif()

message(STATUS "fixture C<->TypeScript composite fixed-array parity smoke passed")
