...
use llvmdsdl_generated::fixtures_union::vendor::choice_1_0::fixtures_union_vendor_Choice as Choice;
fn main() { let mut wire = [1u8, 0, 0, 0, 0]; wire[1..5].copy_from_slice(&5.5f32.to_le_bytes());
  let ok = Choice::get__tag_(&wire) == 1 && Choice::get_real(&wire) == 5.5 && Choice::get_quad(&wire).len() == 4 && Choice::set__tag_(&mut wire, 0) == Ok(());
