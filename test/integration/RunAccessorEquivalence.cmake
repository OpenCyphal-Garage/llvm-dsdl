...
#===----------------------------------------------------------------------===#
#
# A field accessor, in every language, against the body it stands in for.
#
...
# an eighth's fixed array, element by element and one past the end, and a ninth's nested composite,
...
configure_file("${driver_dir}/AccessorEquivalenceDriver.c" "${OUT_DIR}/c_driver.c" @ONLY)
...
configure_file("${driver_dir}/AccessorEquivalenceDriver.cpp" "${OUT_DIR}/cpp_driver.cpp" @ONLY)
execute_process(
...
if(DEFINED CARGO_EXECUTABLE AND EXISTS "${CARGO_EXECUTABLE}")
  ...
  configure_file("${driver_dir}/AccessorEquivalenceCargo.toml.in" "${harness_out}/Cargo.toml" @ONLY)
  configure_file("${driver_dir}/AccessorEquivalenceMain.rs" "${harness_out}/src/main.rs" @ONLY)
...
if(DEFINED GO_EXECUTABLE AND EXISTS "${GO_EXECUTABLE}")
  ...
  file(MAKE_DIRECTORY "${go_out}/accessorequivalence")
  configure_file("${driver_dir}/AccessorEquivalenceMain.go" "${go_out}/accessorequivalence/main.go" @ONLY)
  execute_process(
    COMMAND "${CMAKE_COMMAND}" -E env
      "GOCACHE=${OUT_DIR}/.gocache" "GOMODCACHE=${OUT_DIR}/.gomodcache" "GOFLAGS=-mod=mod"
      "${GO_EXECUTABLE}" build -buildvcs=false -o "${OUT_DIR}/go_equivalence" ./accessorequivalence/
...
if(DEFINED TSC_EXECUTABLE AND EXISTS "${TSC_EXECUTABLE}"
  ...
  configure_file("${driver_dir}/AccessorEquivalenceDriver.ts" "${ts_out}/driver.ts" @ONLY)
...
if(DEFINED PYTHON_EXECUTABLE AND EXISTS "${PYTHON_EXECUTABLE}")
  ...
  _equivalence_run("Python"
    "${PYTHON_EXECUTABLE}" "${driver_dir}/AccessorEquivalenceDriver.py" "${py_out}" "${PY_PACKAGE}")
...
message(STATUS "accessor equivalence passed; legs run: ${legs_text}")
