...
execute_process(
  COMMAND
    "${DSDLC}" --target-language ts --versioned-type-names
      "${fixture_root}"
      --outdir "${ts_out}"
      --ts-module "fixture_ts_bigint_parity"
...
set(c_harness_src "${work_dir}/c_bigint_parity_harness.c")
file(WRITE
  "${c_harness_src}"
  "#include <stdint.h>\n"
  "#include <stdio.h>\n"
  ...
  "  const int8_t ser_rc = demo__BigIntType__serialize_(&in_obj, out_bytes, &out_size);\n"
  "  if ((ser_rc != 0) || (out_size != 16U)) {\n"
  "    return 2;\n"
  "  }\n"
  ...
  "  printf(\"\\n\");\n"
  "\n"
  "  demo__BigIntType out_obj;\n"
  "  out_obj.wide_u = 0U;\n"
  "  out_obj.wide_s = 0;\n"
  "  size_t consumed = out_size;\n"
  "  const int8_t de_rc = demo__BigIntType__deserialize_(&out_obj, out_bytes, &consumed);\n"
  "  if ((de_rc != 0) || (consumed != 16U)) {\n"
  "    return 3;\n"
  "  }\n"
  "  printf(\"%llu %lld %u\\n\", (unsigned long long) out_obj.wide_u, (long long) out_obj.wide_s, (unsigned) consumed);\n"
  "  return 0;\n"
  "}\n"
)

set(c_harness_bin "${work_dir}/c_bigint_parity_harness")
execute_process(
  COMMAND
    "${C_COMPILER}"
      -std=c11
...
if(NOT c_cc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture C bigint parity harness")
endif()
...
if(NOT c_run_result EQUAL 0)
  ...
  message(FATAL_ERROR "fixture C bigint parity harness failed")
endif()

file(WRITE
  "${ts_out}/runtime_bigint_parity_smoke.ts"
  "import { ${ts_type_module} } from \"./index\";\n"
  "\n"
  "const inObj: ${ts_type_module}.BigIntType_1_0 = {\n"
  "  wide_u: 1152921504606846977n,\n"
  "  wide_s: -1152921504606846976n,\n"
  "};\n"
  "const outBytes = ${ts_type_module}.serializeBigIntType_1_0(inObj);\n"
  "if (outBytes.length !== 16) {\n"
  "  throw new Error(\"unexpected serialized size \" + outBytes.length);\n"
  "}\n"
  "console.log(Array.from(outBytes).join(\" \"));\n"
  "\n"
  "const decoded = ${ts_type_module}.deserializeBigIntType_1_0(outBytes);\n"
  "if (typeof decoded.value.wide_u !== \"bigint\" || typeof decoded.value.wide_s !== \"bigint\") {\n"
  "  throw new Error(\"decoded fields are not bigint\");\n"
  "}\n"
  "console.log(decoded.value.wide_u.toString() + \" \" + decoded.value.wide_s.toString() + \" \" + decoded.consumed);\n"
)

file(WRITE
  "${ts_out}/tsconfig-runtime-bigint-parity.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-bigint-parity.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture TypeScript bigint parity smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_bigint_parity_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript bigint parity smoke execution failed")
endif()
...
if(NOT c_output STREQUAL ts_output)
  ...
  message(FATAL_ERROR
    "C vs TypeScript bigint parity mismatch. See ${OUT_DIR}/c-output.txt and ${OUT_DIR}/ts-output.txt.")
endif()

message(STATUS "fixture C<->TypeScript bigint parity smoke passed")
