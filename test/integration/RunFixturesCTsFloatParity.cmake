...
set(c_harness_src "${work_dir}/c_float_parity_harness.c")
file(WRITE
  "${c_harness_src}"
  "#include <stdint.h>\n"
  "#include <stdio.h>\n"
  ...
  "\n"
  "int main(void) {\n"
  "  demo__FloatType in_obj;\n"
  "  in_obj.f32 = 1.5F;\n"
  "  in_obj.f64 = -2.25;\n"
  "\n"
  "  uint8_t out_bytes[12] = {0};\n"
  "  size_t out_size = sizeof(out_bytes);\n"
  "  const int8_t ser_rc = demo__FloatType__serialize_(&in_obj, out_bytes, &out_size);\n"
  "  if ((ser_rc != 0) || (out_size != 12U)) {\n"
  "    return 2;\n"
  "  }\n"
  ...
  "  printf(\"\\n\");\n"
  "\n"
  "  demo__FloatType out_obj;\n"
  "  out_obj.f32 = 0.0F;\n"
  "  out_obj.f64 = 0.0;\n"
  "  size_t consumed = out_size;\n"
  "  const int8_t de_rc = demo__FloatType__deserialize_(&out_obj, out_bytes, &consumed);\n"
  "  if ((de_rc != 0) || (consumed != 12U)) {\n"
  "    return 3;\n"
  "  }\n"
  "  printf(\"%.9g %.17g %u\\n\", (double) out_obj.f32, out_obj.f64, (unsigned) consumed);\n"
  "  return 0;\n"
  "}\n"
)

set(c_harness_bin "${work_dir}/c_float_parity_harness")
execute_process(
  COMMAND
    "${C_COMPILER}"
      -std=c11
...
if(NOT c_cc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture C float parity harness")
endif()
...
if(NOT c_run_result EQUAL 0)
  ...
  message(FATAL_ERROR "fixture C float parity harness failed")
endif()

file(WRITE
  "${ts_out}/runtime_float_parity_smoke.ts"
...
file(WRITE
  "${ts_out}/tsconfig-runtime-float-parity.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-float-parity.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture TypeScript float parity smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_float_parity_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript float parity smoke execution failed")
endif()
...
if(NOT c_output STREQUAL ts_output)
  ...
  message(FATAL_ERROR
    "C vs TypeScript float parity mismatch. See ${OUT_DIR}/c-output.txt and ${OUT_DIR}/ts-output.txt.")
endif()

message(STATUS "fixture C<->TypeScript float parity smoke passed")
