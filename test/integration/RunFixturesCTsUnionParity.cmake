...
execute_process(
  COMMAND
    "${DSDLC}" --target-language ts --versioned-type-names
      "${fixture_root}"
      --outdir "${ts_out}"
      --ts-module "fixture_ts_union_parity"
...
set(c_harness_src "${work_dir}/c_union_parity_harness.c")
file(WRITE
  "${c_harness_src}"
  "#include <stdint.h>\n"
  "#include <stdio.h>\n"
  ...
  "\n"
  "int main(void) {\n"
  "  demo__UnionTag in_obj;\n"
  "  in_obj._tag_ = 1U;\n"
  "  in_obj.second = 0x3456U;\n"
  "\n"
  "  uint8_t out_bytes[4] = {0};\n"
  "  size_t out_size = sizeof(out_bytes);\n"
  "  const int8_t ser_rc = demo__UnionTag__serialize_(&in_obj, out_bytes, &out_size);\n"
  "  if ((ser_rc != 0) || (out_size != 3U)) {\n"
  "    return 2;\n"
  "  }\n"
  "  printf(\"%u %u %u\\n\", (unsigned) out_bytes[0], (unsigned) out_bytes[1], (unsigned) out_bytes[2]);\n"
  "\n"
  "  demo__UnionTag out_obj;\n"
  "  out_obj._tag_ = 0U;\n"
  "  out_obj.first = 0U;\n"
  "  out_obj.second = 0U;\n"
  "  size_t consumed = out_size;\n"
  "  const int8_t de_rc = demo__UnionTag__deserialize_(&out_obj, out_bytes, &consumed);\n"
  "  if ((de_rc != 0) || (consumed != 3U) || (out_obj._tag_ != 1U)) {\n"
  "    return 3;\n"
  "  }\n"
...
set(c_harness_bin "${work_dir}/c_union_parity_harness")
execute_process(
  COMMAND
    "${C_COMPILER}"
      -std=c11
...
if(NOT c_cc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture C union parity harness")
endif()
...
if(NOT c_run_result EQUAL 0)
  ...
  message(FATAL_ERROR "fixture C union parity harness failed")
endif()

file(WRITE
  "${ts_out}/runtime_union_parity_smoke.ts"
...
file(WRITE
  "${ts_out}/tsconfig-runtime-union-parity.json"
  "{\n"
  "  \"compilerOptions\": {\n"
  "    \"target\": \"ES2022\",\n"
  "    \"module\": \"CommonJS\",\n"
...
execute_process(
  COMMAND "${TSC_EXECUTABLE}" -p "${ts_out}/tsconfig-runtime-union-parity.json" --pretty false
...
if(NOT tsc_result EQUAL 0)
  ...
  message(FATAL_ERROR "failed to compile fixture TypeScript union parity smoke")
endif()

file(WRITE "${ts_out}/js/package.json" "{\n  \"type\": \"commonjs\"\n}\n")

execute_process(
  COMMAND "${NODE_EXECUTABLE}" "${ts_out}/js/runtime_union_parity_smoke.js"
...
if(NOT node_result EQUAL 0)
  ...
  message(FATAL_ERROR "TypeScript union parity smoke execution failed")
endif()
...
if(NOT c_output STREQUAL ts_output)
  ...
  message(FATAL_ERROR
    "C vs TypeScript union parity mismatch. See ${OUT_DIR}/c-output.txt and ${OUT_DIR}/ts-output.txt.")
endif()

message(STATUS "fixture C<->TypeScript union parity smoke passed")
