...
///
/// Names that are legal DSDL but collide
...
void appendHeader(std::ostringstream& out)
{
    out << "# llvm-dsdl identifier naming map (golden)\n"
        << "#\n"
        << "# Generated by test/unit/NamingGoldenTests.cpp.\n"
...
void appendCollisions(std::ostringstream& out)
{
    out << "\n== collisions ==\n"
        ...
        << "#   rejected   an output file name or type name; the frontend rejects the pair.\n\n";
...
const std::vector<ScopeScenario>& scopeScenarios()
{
    static const std::vector<ScopeScenario> scenarios = {
        ...
        {"struct_body_reserved",
         ...
         {"fooBar", "serialize", "Serialize", "marshal_binary", "append_binary"},
...
void appendScopes(std::ostringstream& out)
{
    out << "\n== scopes ==\n"
        ...
        << "# claimed-name escapes do the same thing, as the break/break_ rows show.\n\n";
...
std::string renderRoleGolden()
{
    ...
    out << "# llvm-dsdl identifier role policy (golden)\n"
        ...
        << "#\n"
        << "# The 'no' cells are complete: a C or C++ header stem is a file name\n"
