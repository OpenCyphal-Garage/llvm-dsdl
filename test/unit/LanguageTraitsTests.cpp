...
std::string render(const LanguageTraits& row)
{
    ...
    return row.name.str() + ": scopes=" + flag(c.scopes.namespaces) + flag(c.scopes.classes) + flag(c.scopes.modules) +
           ...
           " nested-answers-size=" + flag(b.nestedCallsAnswerSize) + " | namespace-join='" +
           d.definitionName.namespaceJoin.str() + "' version-in-name=" + flag(d.definitionName.versionInTypeName) +
