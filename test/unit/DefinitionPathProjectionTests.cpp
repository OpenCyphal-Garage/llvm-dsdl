...
bool runDefinitionPathProjectionTests()
{
    ...
    if (llvmdsdl::renderVersionedTypeName(Language::TypeScript, "VehicleState", 1, 2) != "VehicleState_1_2")
    {
        std::cerr << "renderVersionedTypeName unexpected TypeScript result\n";
        return false;
    }

    if (llvmdsdl::renderVersionedFileStem(Language::Python, "9AxisIMU", 1, 0) != "_9axis_imu_1_0")
    {
        std::cerr << "renderVersionedFileStem unexpected Python result\n";
        return false;
    }

    const auto nsPath = llvmdsdl::renderNamespaceRelativePath(Language::TypeScript, {"Acme", "FlightControl"});
    ...
    const auto discoveredPath      = llvmdsdl::renderRelativeTypeFilePath(Language::TypeScript, discovered, ".ts");
    ...
    ref.majorVersion        = 2;
    ref.minorVersion        = 3;
    const auto refPath      = llvmdsdl::renderRelativeTypeFilePath(Language::Python, ref, "py");
