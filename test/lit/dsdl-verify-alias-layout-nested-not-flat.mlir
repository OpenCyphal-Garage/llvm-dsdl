// A wire-flat claim over a union. W recurses: a flat run of bytes holds a composite only where
// that type's own form is one, and the width the step states says nothing about it.
//
...
module {
  ...
  dsdl.schema @test_Holder_1_0 attributes {full_name = "test.Holder", major = 1 : i32, minor = 0 : i32, sealed} {
    dsdl.serialization_plan attributes {c_deserialize_symbol = "test__Holder__deserialize_", c_serialize_symbol = "test__Holder__serialize_", c_type_name = "test__Holder", fixed_size, max_bits = 32 : i64, min_bits = 32 : i64, sealed, wire_flat} {
...
// CHECK: error: wire_flat holds but this field's type is not itself wire-flat
