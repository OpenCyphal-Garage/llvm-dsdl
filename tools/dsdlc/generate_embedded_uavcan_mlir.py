...
"""Generate embedded MLIR catalogue for standard UAVCAN namespace."""
...
///
/// @file
/// Auto-generated embedded UAVCAN MLIR catalogue.
///
...
def _run_dsdlc(dsdlc: str, uavcan_root: Path) -> str:
    # --no-embedded-uavcan keeps this hermetic: the catalogue is derived from uavcan_root alone, never
    # from the copy compiled into the dsdlc doing the deriving. It also means a dsdlc whose embedded
    # catalogue is corrupt can still run this, and running this is the fix for that corruption.
    # The catalogue is what `+uavcan.node.Heartbeat.1.0` selects out of, so it has to carry every
    # version. The default generates only the newest of each type, which would make a version
    # selector for anything older fail to resolve against a catalogue that never held it.
...
def _render(mlir_text: str) -> str:
    ...
    # readable). Hash the same bytes so the runtime SHA-256 check verifies what is embedded,
...
def main(argv: Iterable[str]) -> int:
    ...
    if actual == expected:
        print("embedded UAVCAN MLIR catalogue is up to date")
        return 0
