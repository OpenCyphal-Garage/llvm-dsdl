...
def _detect_traits(repo_root: Path, cfg: Dict[str, object], text: str) -> Dict[str, bool]:
    ...
    if kind == "c":
        ...
        traits["lower_pass"] = _has_any(_load_text(repo_root, "tools/dsdlc/main.cpp"), [r"addLowerDSDLBodiesPipeline\("])
...
def _write_markdown(path: Path, report: Dict[str, object]) -> None:
    ...
    lines: List[str] = []
    lines.append("# Shared-Infrastructure Consistency Lint")
    lines.append("")
    lines.append("Generated by `tools/convergence/convergence_report.py`.")
    lines.append("")
    lines.append(
        "> **What this is.** A *static lint* that verifies each backend emitter references "
        ...
        "equivalence is enforced separately: every backend's bodies are translations of the "
        "plan bodies (`ctest -L backend-contract`), and *wire bytes* are checked by the "
        "parity and malformed-input gates, which consume executed test pass/fail. "
    ...
    lines.append("")
    lines.append("| Backend | Markers present | Classes shared / total |")
    lines.append("| --- | ---: | ---: |")
    ...
    lines.append("")
    lines.append(f"Project floor (minimum markers-present score across backends): `{report['project_floor_score']}`")
    lines.append("")
