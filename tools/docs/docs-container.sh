...
# docs/showroom is generated by the `showroom-docs` build target and is not committed; this image
# renders the site but cannot build dsdlc, so it cannot produce those pages. Without them mkdocs
# aborts on the nav entry for the showroom overview, which reads as a broken site rather than as a
# missing build step. Say so up front. A warning rather than an error: the mount may be an unrelated
# MkDocs project, which is a supported use of this script.
warn_if_showroom_missing() {
    local dir="$1"
    [ -d "${dir}/examples/showroom" ] || return 0
    [ -f "${dir}/docs/showroom/index.md" ] && return 0
    warn "${dir}/docs/showroom is missing; mkdocs --strict will abort on the Showroom nav entry."
    warn "generate it first, e.g.:"
    warn "  cmake --build <build-dir> --config RelWithDebInfo --target showroom-docs"
}
