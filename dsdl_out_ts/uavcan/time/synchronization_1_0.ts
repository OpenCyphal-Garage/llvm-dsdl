...
export const DSDL_FULL_NAME = "uavcan.time.Synchronization";
...
export interface Synchronization {
  ...
  previous_transmission_timestamp_microsecond: bigint;
}

// [second]
// Publication period limits.
// A master should not change its publication period while running.
export const SYNCHRONIZATION_MAX_PUBLICATION_PERIOD = 1;
...
export const SYNCHRONIZATION_PUBLISHER_TIMEOUT_PERIOD_MULTIPLIER = 3;

export function serializeSynchronization(value: Synchronization): Uint8Array {
  const out = new Uint8Array(7);
  let offsetBits = 0;
  const mlir_llvmdsdl_plan_capacity_check__uavcan_time_Synchronization_1_0 = (capacityBits: number): boolean => 56 <= capacityBits;
  const mlir_llvmdsdl_plan_scalar_unsigned__uavcan_time_Synchronization_1_0__0__ser = (value: number | bigint): number | bigint => {
  ...
  if (!mlir_llvmdsdl_plan_capacity_check__uavcan_time_Synchronization_1_0(out.length * 8)) {
    throw new Error("serialization buffer too small");
  }
  dsdlRuntime.writeUnsigned(out, offsetBits, 56, mlir_llvmdsdl_plan_scalar_unsigned__uavcan_time_Synchronization_1_0__0__ser(value.previous_transmission_timestamp_microsecond), false);
  offsetBits += 56;
  const usedBytes = dsdlRuntime.byteLengthForBits(offsetBits);
  return out.subarray(0, usedBytes);
}

export function deserializeSynchronization(bytes: Uint8Array): { value: Synchronization; consumed: number } {
  const mlir_llvmdsdl_plan_scalar_unsigned__uavcan_time_Synchronization_1_0__0__deser = (value: number | bigint): number | bigint => {
    const raw = (typeof value === "bigint") ? value : BigInt(Math.trunc(value));
    const masked = raw & 72057594037927935n;
    return (typeof value === "bigint") ? masked : Number(masked);
  };

  const value = {} as Synchronization;
  let offsetBits = 0;
  const previous_transmission_timestamp_microsecondRaw = dsdlRuntime.readUnsignedBigInt(bytes, offsetBits, 56);
  value.previous_transmission_timestamp_microsecond = BigInt(mlir_llvmdsdl_plan_scalar_unsigned__uavcan_time_Synchronization_1_0__0__deser(previous_transmission_timestamp_microsecondRaw));
  offsetBits += 56;
