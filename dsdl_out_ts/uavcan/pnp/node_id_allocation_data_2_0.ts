...
export const DSDL_FULL_NAME = "uavcan.pnp.NodeIDAllocationData";
...
export interface NodeIDAllocationData {
...
export function serializeNodeIDAllocationData(value: NodeIDAllocationData): Uint8Array {
  const out = new Uint8Array(18);
  let offsetBits = 0;
  const mlir_llvmdsdl_plan_capacity_check__uavcan_pnp_NodeIDAllocationData_2_0 = (capacityBits: number): boolean => 144 <= capacityBits;
  const mlir_llvmdsdl_plan_scalar_unsigned__uavcan_pnp_NodeIDAllocationData_2_0__2__ser = (value: number | bigint): number | bigint => {
  ...
  if (!mlir_llvmdsdl_plan_capacity_check__uavcan_pnp_NodeIDAllocationData_2_0(out.length * 8)) {
  ...
  for (let i = 0; i < 16; ++i) {
    dsdlRuntime.writeUnsigned(out, offsetBits, 8, mlir_llvmdsdl_plan_scalar_unsigned__uavcan_pnp_NodeIDAllocationData_2_0__2__ser(unique_idArray[i]), true);
    offsetBits += 8;
  }
...
export function deserializeNodeIDAllocationData(bytes: Uint8Array): { value: NodeIDAllocationData; consumed: number } {
  const mlir_llvmdsdl_plan_scalar_unsigned__uavcan_pnp_NodeIDAllocationData_2_0__2__deser = (value: number | bigint): number | bigint => {
    const raw = (typeof value === "bigint") ? value : BigInt(Math.trunc(value));
    const masked = raw & 255n;
    return (typeof value === "bigint") ? masked : Number(masked);
  };

  const value = {} as NodeIDAllocationData;
  let offsetBits = 0;
  ...
  for (let i = 0; i < 16; ++i) {
    ...
    unique_idArray[i] = Number(mlir_llvmdsdl_plan_scalar_unsigned__uavcan_pnp_NodeIDAllocationData_2_0__2__deser(unique_idItemRaw));
    offsetBits += 8;
  }
